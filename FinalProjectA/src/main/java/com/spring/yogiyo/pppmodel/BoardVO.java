...
public class BoardVO {
   ...
   // === #124. 답변형 게시판을 위한 필드추가하기
   //          먼저, 오라클에서 tblComment 테이블과 boardSeq 테이블을 drop 한 후에
   //          tblComment 테이블과 boardSeq 테이블을 재생성 한 이후에 
   ...
   */
   ...
   /*
      === #138. 파일을 첨부하도록 VO 수정하기
              먼저, 오라클에서 tblBoard 테이블에
              3개 컬럼(fileName, orgFilename, fileSize)을 추가한 다음에 아래의 작업을 한다.
   */
   private String fileName;       // WAS(톰켓)에 저장될 파일명(2019072509271512312.png)
   private String orgFilename;      // 진짜 파일명(강아지.png) // 사용자가 파일을 업로드 하거나 파일을 다운로드 할때 사용되어지는 파일명
   private String fileSize;      // 파일크기
   
   private MultipartFile attach;   // 진짜 파일 ==> WAS(톰켓) 디스크에 저장됨.
   ...
   public BoardVO(String seq, String fk_email, String name, String subject, String content, String pw,
