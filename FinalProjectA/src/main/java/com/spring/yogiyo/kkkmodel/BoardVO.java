...
public class BoardVO {
	...
	//			  먼저, 오라클에서 tblComment 테이블 과 tblBoard 테이블을 drop 한 후에
	//			 tblComment 테이블 과 tblBoard 테이블을 재생성 한 이후에 아래처럼 해야 한다.
	...
	private String fileName;		// WAS(톰캣)에 저장 될 파일명( )
	private String orgFilename;		// 진짜 파일명(강아지.png)  // 사용자가 파일을 업로드 하거나 파일을 다운로드 할 때 사용되어지는 파일명
	private String fileSize;		// 파일크기
	
	private MultipartFile attach;   // 첨부되어질 실제 파일 ==> WAS(톰캣) 디스크에 저장됨.  (위 3개는 데이터베이스에 기록하기 위한 것)
