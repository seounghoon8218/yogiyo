	...
	$(document).ready(function(){
		...
		$("#btnWrite").click(function(){
		    ...
		    */    
			...
			frm.method = "POST";	// 파일을 첨부할 경우이라면 반드시 POST 이어야만 가능하다. GET이라면 파일첨부가 불가능하다.
			frm.action = "<%=ctxPath%>/pppaddEnd.yo";
			frm.submit();
		});
...
<%-- enctype="multipart/form-data" 를 해우어야만 파일첨부가 되어진다. --%>
	<form name="addFrm" enctype="multipart/form-data">
		<table id="table">
			<tr>
				<th>성명</th>
				<td>
					<input type="hidden" name="fk_email" value="${sessionScope.loginuser.email }" readonly="readonly" />
					<input type="text" name="name" value="${sessionScope.loginuser.name }" class="short" readonly="readonly" />
				</td>
			</tr>
			<tr>
				<th>제목</th>
				<th><input type="text" name="subject" id="subject" /></th>
			</tr>
			<tr>
				<th>글내용</th>
		...
		<!-- === #127.답변글쓰기인 경우 -->
		<!-- 부모글의 seq값인 fk_seq 값과  부모글의 groupno 값과 부모글의 depthno 값을 hidden 타입으로 보내준다. -->
